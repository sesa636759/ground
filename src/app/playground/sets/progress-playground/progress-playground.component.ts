...
export class DmProgressPlaygroundComponent extends BasePlaygroundComponent {
  ...
  updateConfig() {
    ...
    if (this.pgConfig.strokeWidth !== 10)
      code += `  [stroke-width]="${this.pgConfig.strokeWidth}"\n`;
    ...
    if (this.pgConfig.type !== 'line') {
      ...
      if (this.pgConfig.gapPosition !== 'top')
    ...
    if (this.pgConfig.strokeColor !== '#1890ff')
      code += `  stroke-color="${this.pgConfig.strokeColor}"\n`;
    if (this.pgConfig.trailColor !== '#f0f0f0')
    ...
    if (this.pgConfig.showTimeEstimate) {
      ...
      if (this.pgConfig.estimatedTime > 0)
        code += `  [estimated-time]="${this.pgConfig.estimatedTime}"\n`;
    }

    if (this.pgConfig.decimalPrecision > 0)
      code += `  [decimal-precision]="${this.pgConfig.decimalPrecision}"\n`;
    if (this.pgConfig.percentageColor)
      code += `  percentage-color="${this.pgConfig.percentageColor}"\n`;
